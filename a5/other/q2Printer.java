...
private void  clearBuffer(){
	...
	for(int i=0; i<numberOfPhil;i++){
		if(bufferArray[i] != 'x'){
			print_tabs(tabs); //print tabs
			System.out.print(bufferArray[i]); //print state
			 if (bufferArray[i] == q2Philosopher.States.Waiting.val || bufferArray[i] == q2Philosopher.States.Eating.val) {
			 		System.out.print(leftValue[i] + "," + rightValue[i]); //print left and right values if condition met
			}
			tabs=1;
		} else {
			tabs++;
